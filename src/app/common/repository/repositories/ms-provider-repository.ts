...
import { IProviderMethods } from './../interfaces/ms-provider-methods';

@Injectable({ providedIn: 'root' })
export class ProviderRepository<T> implements IProviderMethods<T> {
  ...
  private buildRoute(route: string) {
    ...
    paths.shift();
    if (paths.length === 0) {
      return `${environment.API_URL}provider/api/v1/${route}`;
    }
