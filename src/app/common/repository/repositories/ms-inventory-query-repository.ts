...
import { IInventortQueryMethods } from '../interfaces/ms-inventory-query-methods';

@Injectable({ providedIn: 'root' })
export class InventoryQueryRepository<T> implements IInventortQueryMethods<T> {
  ms: string = `${environment.API_URL}inventoryquery/api/v1/`;

  constructor(public readonly httpClient: HttpClient) {}

  getAll(route: string, _params?: ListParams, filter?: string) {
    const fullRoute = `${this.ms}${route}`;
    const params = this.makeParams(_params, filter);
