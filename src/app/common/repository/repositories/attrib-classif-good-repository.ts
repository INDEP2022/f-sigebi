...
import { IAttribClassifGoodMethods } from '../interfaces/attrib-classif-good-methods';
import { ListParams } from '../interfaces/list-params';
@Injectable({ providedIn: 'root' })
export class AttribClassifGoodMethodsRepository<T>
  implements IAttribClassifGoodMethods<T>
{
  ms: string = `${environment.API_URL}goodsquery/api/v1`;
  ...
  private buildRoute(route: string) {
    ...
    paths.shift();
    if (paths.length === 0) {
      return `${environment.API_URL}goodsquery/api/v1/${route}`;
    }
