export const SELECT_GOODS_EYE_VISIT_COLUMNS = {
  goodGrouper: {
    title: 'Nombre del Bien Agrupador',
    type: 'string',
    sort: false,
  },
  instanceDate: {
    title: 'Fecha Hora Inicio',
    type: 'string',
    sort: false,
  },
  instancebpel: {
    title: 'Fecha Hora Fin',
    type: 'string',
    sort: false,
  },
  proceedingsId: {
    title: 'Expediente',
    type: 'string',
    sort: false,
  },
  applicationId: {
    title: 'No. Solicitud',
    type: 'string',
    sort: false,
  },
  goodId: {
    title: 'No. Gestión',
    type: 'string',
    sort: false,
  },
  amount: {
    title: 'Cantidad Reservada',
    type: 'string',
    sort: false,
  },
  descriptionGood: {
    title: 'Descripción',
    type: 'string',
    sort: false,
  },
  numberGoodsiab: {
    title: 'No. INDEP',
    type: 'string',
    sort: false,
  },
  inventoryNumber: {
    title: 'No. Inventario',
    type: 'string',
    sort: false,
  },
  origin: {
    title: 'Origen',
    type: 'string',
    sort: false,
  },
  jobNumber: {
    title: 'No. Oficio',
    type: 'string',
    sort: false,
  },
  proceedingsType: {
    title: 'Tipo Expediente',
    type: 'string',
    sort: false,
  },
  uniqueKey: {
    title: 'Clave Única',
    type: 'string',
    sort: false,
  },
  //unitExtent
  unitExtent: {
    title: 'Unidad de Medida',
    type: 'string',
    sort: false,
  },
  statePhysical: {
    title: 'Estado Físico',
    type: 'string',
    sort: false,
  },
  stateConservation: {
    title: 'Estado Conservación',
    type: 'string',
    sort: false,
  },
  //fractionId
  fractionDescrip: {
    title: 'Fracción',
    type: 'string',
    sort: false,
  },
  destination: {
    title: 'Destino',
    type: 'string',
    sort: false,
  },
  //delegationRegionalId
  delegationDescrip: {
    title: 'Delegación',
    type: 'string',
    sort: false,
  },
  codeStore: {
    title: 'Código Almacén',
    type: 'string',
    sort: false,
  },
};

export const SELECTED_GOOD_REVIEW = {
  goodGrouper: {
    title: 'Nombre del Bien Agrupador',
    type: 'string',
    sort: false,
  },
  resultTaxpayer: {
    title: 'Resultado Contribuyente',
    type: 'string',
    sort: false,
  },
  observationsResult: {
    title: 'Observación Visita Ocular',
    type: 'string',
    sort: false,
  },
  applicationId: {
    title: 'No. Solicitud',
    type: 'string',
    sort: false,
  },
  proceedingsId: {
    title: 'Expediente',
    type: 'string',
    sort: false,
  },
  goodId: {
    title: 'No. Gestión',
    type: 'string',
    sort: false,
  },
  amount: {
    title: 'Cantidad Reservada',
    type: 'string',
    sort: false,
  },
  descriptionGood: {
    title: 'Descripción',
    type: 'string',
    sort: false,
  },
  numberGoodsiab: {
    title: 'No. Sae',
    type: 'string',
    sort: false,
  },
  inventoryNumber: {
    title: 'No. Inventario',
    type: 'string',
    sort: false,
  },
  origin: {
    title: 'Origen',
    type: 'string',
    sort: false,
  },
  jobNumber: {
    title: 'No. Oficio',
    type: 'string',
    sort: false,
  },
  proceedingsType: {
    title: 'Tipo Expediente',
    type: 'string',
    sort: false,
  },
  uniqueKey: {
    title: 'Clave Única',
    type: 'string',
    sort: false,
  },
  //unitExtent
  unitExtentDescrip: {
    title: 'Unidad de Medida',
    type: 'string',
    sort: false,
  },
  statePhysical: {
    title: 'Estado Físico',
    type: 'string',
    sort: false,
  },
  stateConservation: {
    title: 'Estado Conservación',
    type: 'string',
    sort: false,
  },
  //fractionId
  fractionDescrip: {
    title: 'Fracción',
    type: 'string',
    sort: false,
  },
  destination: {
    title: 'Destino',
    type: 'string',
    sort: false,
  },
  //delegationRegionalId
  delegationDescrip: {
    title: 'Delegación',
    type: 'string',
    sort: false,
  },
  codeStore: {
    title: 'Código Almacén',
    type: 'string',
    sort: false,
  },
};

export const SELECTED_GOOD_VIEW = {
  origin: {
    title: 'Origen',
    type: 'string',
    sort: false,
  },
  descriptionGood: {
    title: 'Descripción',
    type: 'string',
    sort: false,
  },
  uniqueKey: {
    title: 'Clave Única',
    type: 'string',
    sort: false,
  },
  amount: {
    title: 'Cantidad Reservada',
    type: 'string',
    sort: false,
  },

  applicationId: {
    title: 'No. Solicitud',
    type: 'string',
    sort: false,
  },
  proceedingsId: {
    title: 'Expediente',
    type: 'string',
    sort: false,
  },
  goodId: {
    title: 'No. Gestión',
    type: 'string',
    sort: false,
  },

  numberGoodsiab: {
    title: 'No. Sae',
    type: 'string',
    sort: false,
  },
  inventoryNumber: {
    title: 'No. Inventario',
    type: 'string',
    sort: false,
  },

  jobNumber: {
    title: 'No. Oficio',
    type: 'string',
    sort: false,
  },
  proceedingsType: {
    title: 'Tipo Expediente',
    type: 'string',
    sort: false,
  },

  //unitExtent
  unitExtentDescrip: {
    title: 'Unidad de Medida',
    type: 'string',
    sort: false,
  },
  statePhysical: {
    title: 'Estado Físico',
    type: 'string',
    sort: false,
  },
  stateConservation: {
    title: 'Estado Conservación',
    type: 'string',
    sort: false,
  },
  //fractionId
  fractionDescrip: {
    title: 'Fracción',
    type: 'string',
    sort: false,
  },
  destination: {
    title: 'Destino',
    type: 'string',
    sort: false,
  },
  //delegationRegionalId
  delegationDescrip: {
    title: 'Delegación',
    type: 'string',
    sort: false,
  },
  codeStore: {
    title: 'Código Almacén',
    type: 'string',
    sort: false,
  },
};
