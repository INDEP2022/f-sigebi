...
import { TRANSFERENTES } from '../columns';

@Component({
  selector: 'app-transferents',
  templateUrl: './transferents.component.html',
  styles: [],
})
export class TransferentsComponent extends BasePage implements OnInit {
    ...
    this.transferenteService.getAll(params).subscribe({
      next: value => {
        let result = value.data.map(item => {
          item['idAndDescription'] = item.id + ' - ' + item.keyTransferent;
        });
