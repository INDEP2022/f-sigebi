...
export class ExportGoodsDonationComponent extends BasePage implements OnInit {
  ...
  // Se declara e inicializa varias variables locale
  n_Inva: number = 0;
  n_Acta: number = 0;
  n_Acum: number = 0;
  LNU_REGISTROS: number = 0;
  V_NO_BIEN_REFERENCIA: string;
  V_NO_BIEN_PADRE_PARCIALIZACION: string;
  V_NO_ACTA: string;
  V_NO_EXPEDIENTE: string;

  // Propiedades procedure Actualización Inventario

  L_PATH: string;
  vc_filtro: string;
  LFiarchivo: any;
  LSt_archivo_destino: string;
  lnu_reg_act: number;
  lnu_ciclo: number;
  LNU_NO_TRANSFERENTE: number;
  LNU_NO_EMISORA: number;
  ...
  getall() {
    ...
    /* this.detailProceeDelRecService.getProceding().subscribe({
      next: response => {
        console.log('getMax -> ', response);
      },
    });
  
    this.detailProceeDelRecService.getProceding2().subscribe({
      next: response => {
        console.log('getProceding2 -> ', response);
      },
    });
  
    this.expedientSamiService.getexpedient2().subscribe({
      next: response => {
        console.log('getexpedient2 -> ', response);
      },
    });*/
  }
...
/*getCatalog() {
    this.delegationService.getCatalog().subscribe({
      next: response => {
        console.log('consele -> ', response);
      },
    });
