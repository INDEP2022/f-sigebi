  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    this.filterParams.pipe(takeUntil(this.$unSubscribe)).subscribe({
      next: () => {
        if (this.banks) this.getParameterByDetails();
      },
    });
  ...
  validateRowDetail(details: any) {
    alert(this.details.date);
  }
  ...
  getParameterByDetails2() {
    ...
    this.goodParametersService.getById(param).subscribe({
      next: resp => {
        ...
        //console.log("data formulario -> ",this.detailsMovement);
  ...
  pupInterestsDetail() {
    ...
    this.accountService.pupInterestsDetail(model).subscribe({
      next: response => {
        ...
        console.log('data -> ', this.detailsMovement);
      },
      error: error => {
