...
import { financialIndicators, financialAtribs} from '../select-attributes/info';
...
export class PaEFiCSummaryFinancialInfoComponent extends BasePage implements OnInit {
  ...
  openModal(): void {
    ...
    modalRef.content.data.subscribe(data => {
      if(data)
        this.financialAtribs=data.checkedListFA;
  ...
  removeItem(id:string,tags:string): void {
    if(tags == 'financialAtribs') {
      this.financialAtribs.filter(fa=>fa.id==id).map(a=>{
        a.isSelected=false
      });
      this.checkedListFA=this.financialAtribs.filter(fa=>fa.isSelected);
    }

    this.financialIndicators.filter(fi=>fi.id==id).map(i=>{
      i.isSelected=false
    });
    this.checkedListFI=this.financialIndicators.filter(fi=>fi.isSelected);
  }
