...
export class PropertyRegistrationComponent extends BasePage implements OnInit {
  ...
  searchExpedient(event: any) {
    ...
    this.expedientServices.getById(numberFile).subscribe({
      next: response => {
        ...
        this.searchGoods(this.paramsSubject, this.idExpedient);
      },
      error: err => {
  ...
  uploadTableMenaje(good: IGood) {
    if (good) {
      ...
      this.searchGood(new ListParams(), good.id);
    } else {
  ...
  searchGoods(
    ...
    this.goodServices.getByExpedient(this.idExpedientSearch, params).subscribe({
      next: response => {
    ...
    this.menageServices.getMenaje(paramsF.getParams()).subscribe({
      next: response => {
        ...
        if (response.count > 0) {
          ...
          this.loading = false;
          this.totalItems = 0;
          this.searchGoods(this.paramsSubject, this.idExpedientSearch);
        }
      },
      error: err => {
        this.alert('warning', 'Bien sin Menajes Asociados', ``);
  ...
  searchGood(params: ListParams, id?: number) {
    if (id) {
