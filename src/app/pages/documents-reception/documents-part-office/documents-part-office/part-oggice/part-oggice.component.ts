...
export class PartOggiceComponent extends BasePage implements OnInit {
  ...
  buildFilters(): void {
    ...
    if (startDate !== null && endDate !== null) {
      const startTemp = `${startDate.getFullYear()}-0${startDate.getUTCMonth() + 1
        }-0${startDate.getDate()}`;
      const endTemp = `${endDate.getFullYear()}-0${endDate.getUTCMonth() + 1
        }-0${endDate.getDate()}`;
  ...
  acptionBienes() {
    ...
    const modalRef = this.modalService.show(DocumentsPartOfficeModalTableComponent, config);
  }
