...
//Models
import { IManagementArea, IProceduremanagement } from 'src/app/core/models/ms-proceduremanagement/ms-proceduremanagement.interface';
...
import { FLYER_HISTORY_COLUMNS } from '../documents-part-office/utils/flyer-history-columns';
import { INDICATORS_HISTORY_COLUMNS } from '../documents-part-office/utils/indicators-history-columns';
import {
  ANTECEDENTE_TITLE,
  BIENES_TITLE,
  FLYER_HISTORY_TITLE,
  ...
  RELATED_FOLIO_TITLE,
} from '../documents-part-office/utils/modal-titles';
import { RELATED_FOLIO_COLUMNS } from '../documents-part-office/utils/related-folio-columns';
import {
  CONFIRM_CANCEL,
  CONFIRM_FINISH,
  NO_FLYER_NUMBER,
  NO_INDICATORS_FOUND,
} from '../documents-part-office/utils/documents-part-office-messages';
import {
...
import { DocuentsReceptionRegisterFormChanges } from '../../flyers/documents-reception-register/interfaces/documents-reception-register-form';
...
export class DocumentsPartOfficeComponent extends BasePage implements OnInit {
  ...
  dataTaR: any[] = [];
  data: any[] = [];
  dataTAsuntos: any[] = [{ id: 1, descripcion: "Acta Circunstanciada" },
  ...
  ngOnInit(): void {
    ...
    this.tasuntos$ = new DefaultSelect(this.dataTAsuntos, this.dataTAsuntos.length);
  ...
  getProceduraManagment(val: number) {
    ...
    this.loading = true;
    if (this.rdoD) {
      this.params2.getValue()["filter.actualDate"] = this.dateFormat(new Date);
    }
    if (this.rdoR) {
      this.params2.getValue()["filter.actualDate"] = `$btw:${this.dateFormat(new Date(this.startDate2.value))},${this.dateFormat(new Date(this.endDate2.value))}`;
    }
    if (this.rdoT) {
      this.params2.getValue()["filter.actualDate"] = `$gt:${this.dateFormat(new Date("2015-01-01"))}`;
    }
    ...
    this.procedureManagementService.getAllFiltered(this.params2.getValue()).subscribe({
      next: (data: any) => {
  ...
  register() {
    if (this.filterForm.valid) {
      ...
      this.onLoadToast("warning", "Advertencia", "No a seleccionado todos los campos requeridos");
  ...
  cdeleteOP(event: any) {
    this.alertQuestion(
      ...
      if (question.isConfirmed) {
        this.dataTa = this.dataTa.filter(dat => dat.dailyConsecutiveNumber !== event.data.dailyConsecutiveNumber);
        this.dataTable.load(this.dataTa);
        this.dataTable.refresh();
  ...
  udpateReg() {
    ...
    let idx = this.dataTa.findIndex(dat => dat.dailyConsecutiveNumber == this.filterForm.get('dailyConsecutiveNumber').value);
    //console.log(idx);
  ...
  openForm(datos?: IProceduremanagement) {
    if (datos.status == "OPI" || datos.status == "OPP" || datos.status == "I") {
      ...
      this.onLoadToast('warning', 'Aviso', 'No se puede realizar la modificación. Verifica que el trámite esté en OPI, OPP.');
    }
