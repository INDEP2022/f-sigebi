...
export class AppraisalConsultationComponent extends BasePage implements OnInit {
  ...
  constructor(private excelService: ExcelService, private fb: FormBuilder, private appraiseService:AppraiseService) {
    super();
  ...
  getAppraiseByFilters2(): void {
    ...
    this.filterField();
    this.appraiseService.getAppraiseByFilters(this.filterParams.getValue().getParams()).subscribe({
      next: response => {
        this.show = false;
  ...
  filterField(){
    this.form.controls['idEvent'].value?this.filterParams.getValue().addFilter('idEvent', this.form.get('idEvent').value):null;
    this.form.controls['idAppraisal'].value?this.filterParams.getValue().addFilter('idAppraisal', this.form.get('idAppraisal').value):null;
    this.form.controls['statusAppraisal'].value?this.filterParams.getValue().addFilter('statusAppraisal', this.form.get('statusAppraisal').value):null;
    this.form.controls['keyAppraisal'].value?this.filterParams.getValue().addFilter('keyAppraisal', this.form.get('keyAppraisal').value):null;
    this.form.controls['keyOffice'].value?this.filterParams.getValue().addFilter('keyOffice', this.form.get('keyOffice').value):null;
    this.form.controls['statusAppraisal'].value?this.filterParams.getValue().addFilter('statusAppraisal', this.form.get('statusAppraisal').value):null;
    this.form.controls['noGood'].value?this.filterParams.getValue().addFilter('noGood', this.form.get('noGood').value):null;
  }
