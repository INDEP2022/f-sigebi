...
export class TypeEventModalComponent extends BasePage implements OnInit {
  ...
  constructor(private modalRef: BsModalRef, private fb: FormBuilder, private typeEventXterComerService: TypeEventXterComerService, private comerTpEventosService:ComerTpEventosService) {
    super();
  }
  ...
  private prepareForm() {
    ...
    if (this.typeEvents != null) {
      this.idTypeEvent = this.typeEvents.typeEventId as unknown as IComerTpEvent;
  ...
  getTypeEvents(params: ListParams){
    this.comerTpEventosService.getAll(params).subscribe({
      next: data => (this.typeEventsSelect = new DefaultSelect(data.data, data.count)),
    });
  }
  ...
  update() {
    ...
    this.typeEventXterComerService
      .update(this.typeEvents.thirdPartyId, this.typeEvents.typeEventId, this.typeEvent3erForm.value)
      .subscribe({
  ...
  create() {
    this.loading = true;
    this.typeEventXterComerService.create(this.typeEvent3erForm.value).subscribe({
      next: data => this.handleSuccess(),
      error: error => (this.loading = false),
    });
  }
