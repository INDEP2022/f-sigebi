...
@Component({
  selector: 'app-c-bm-r-rrpr-c-remittances-recorded-region',
  templateUrl: './c-bm-r-rrpr-c-remittances-recorded-region.component.html',
  styles: [
  ]
})
export class CBmRRrprCRemittancesRecordedRegionComponent extends BasePage implements OnInit {
  ...
  constructor(private fb: FormBuilder, private modalService: BsModalService, private sanitizer: DomSanitizer,) {
    super();
    this.today = new Date();
  ...
  getCoordinations(params: ListParams) {
    if (params.text == '') {
      this.coordinationsItems = new DefaultSelect(this.coordinationsTestData, 5);
    } else {
