...
export class CatTypesOfGoodsSubTypeComponent extends BasePage implements OnInit {
  edit: boolean = false;
  ...
  prepareForm() {
    ...
    if (this.data != null) {
      ...
      this.typeGoodsForm.controls['idTypeGood'].setValue(this.data.idTypeGood.id);
    } else {
  ...
  update() {
    ...
    this.goodSubTypesService.updateByIds(ids, this.typeGoodsForm.getRawValue()).subscribe({
  ...
  create() {
    this.loading = true;
    this.goodSubTypesService.create(this.typeGoodsForm.getRawValue()).subscribe({
      next: data => this.handleSuccess(),
      error: error => (this.loading = false),
    });
  }
