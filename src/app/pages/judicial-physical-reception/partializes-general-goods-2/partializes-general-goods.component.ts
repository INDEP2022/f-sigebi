  ...
  implements OnInit
{
  ...
  checkSum(pindica: string): boolean {
    // debugger;
    let vsum: number;
    let vban: boolean;
  ...
  private fillRow(newImporte: number) {
  ...
  private async applyContinue(v_importe: number) {
    let v_estatus: string,
      vaccion: string,
      vproextdom: string,
      vno_acta: number,
      v_verif_des: string;
  ...
  async apply() {
    ...
    const checkSum = this.checkSum('O');
    if (!checkSum) {
      this.loading = false;
      return;
    }
