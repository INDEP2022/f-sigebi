...
@Component({
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination">
            <li class="page-item" [ngClass]="currentPage==1?'disabled':''" [ngStyle]="{cursor:currentPage==1?'':'pointer'}">
            ...
            <ng-container *ngFor="let page of pages">
                <li class="page-item" [ngClass]="page==currentPage?'active':''"
                    [ngStyle]="{cursor:page==currentPage?'':'pointer'}" [attr.aria-current]="page"
                    *ngIf="page>=startPage&&page<=endPage">
                    <span class="page-link" (click)="page==currentPage?'':selectPage(page)">{{page}}</span>
                </li>
            </ng-container>
            <li class="page-item" [ngClass]="currentPage==pages.length?'disabled':''"
